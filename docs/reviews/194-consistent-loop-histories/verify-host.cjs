...
const output = process.env.EVALENS_194_REPORT || '/private/tmp/evalens-194/source';
const expectedRoot = process.env.EVALENS_194_ROOT || '/Users/mbjarland/projects/evalens-worktrees/194-consistent-loop-histories';
const cases = [
  {name:'body', lines:[/v: 1, 2, 3 · 3 iterations.*u: 4, 8, 12 · 3 iterations/], hoverLine:0, hover:/u = 4, 8, 12/},
  {name:'filtered', lines:[/v: 0, 1, 2, 3, 4 · 5 iterations.*kept: 4, 12 · 2 iterations/], hoverLine:0, hover:/bound on 2 of 5 iterations/},
  {name:'constant', lines:[/v: 0, 1, 2 · 3 iterations.*unchanged: 7 · 3 iterations/], hoverLine:0, hover:/unchanged over 3 iterations/},
  {name: 'uniform', lines: [
    /x: 0, 1, 2, 3, 4, …, 99 · 100 iterations/,
  ...
  {name: 'uneven', lines: [/x: 1, 3, 0, 2 · 4 iterations/, /y: 0, 0, 1, 0, 1 · 4 runs · 5 iterations total/], hover: /5 iterations total across 4 loop runs/},
  {name: 'empty', lines: [/x: 0, 1 · 2 iterations/, /y: \(no iterations\) · 2 runs · 0 iterations total/], hover: /0 iterations total across 2 loop runs/},
  {name: 'unreached', lines: [/x: \(no iterations\)/, /y: \(not reached\)/], hover: /Loop not reached/},
  {name: 'single', lines: [/x: 0 · 1 iteration/, /y: 0, 1, 2 · 3 iterations/], hover: /3 iterations/},
  {name: 'shadowed', lines: [/x: 1, 2 · 2 iterations/, /x: 7, 8, 7, 8 · 2 runs · 4 iterations total/, /x: 9, 9, 9, 9 · 4 runs · 4 iterations total/], hover: /4 iterations total across 2 loop runs/},
];
(async () => {
  ...
  try {
    ...
    for (const test of cases) {
      const file = '/private/tmp/evalens-194/workspace/' + test.name + '.py';
      ...
      const hover = await api({op:'hover', line:test.hoverLine ?? 1, character:8});
      ...
      await api({op:'cursor', line:test.hoverLine ?? 1, character:8});
      await sleep(200);
