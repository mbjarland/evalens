...
exports.api=async d=>{const x=await(await fetch('http://127.0.0.1:9415',{method:'POST',body:JSON.stringify(d)})).json();assert(x.ok,JSON.stringify(x));return x.result;};
...
exports.connect=async()=>{
 ...
 const browser=await connect({browserURL:'http://127.0.0.1:9414',defaultViewport:null});
