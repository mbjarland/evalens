...
const repo=process.argv[2]||'/Users/mbjarland/projects/evalens-worktrees/201-loop-source-before-visualization';
...
const out='/private/tmp/evalens-201';
(async()=>{
 ...
 try{
  ...
  const fixture=fs.readFileSync(repo+'/docs/reviews/196-marketplace-page-refresh/fixtures/nested-loops.py','utf8').trim();
  const example=await page.$eval('.itemDetails .markdown',e=>{
   ...
   const code=nodes.find(n=>n.tagName==='PRE')?.querySelector('code');
   const image=nodes.flatMap(n=>n.tagName==='IMG'?[n]:[...n.querySelectorAll('img')]).find(i=>i.alt.startsWith('Nested loop explorer'));
   return {source:code?.textContent.trim(),codeBeforeImage:!!(code&&image&&(code.compareDocumentPosition(image)&Node.DOCUMENT_POSITION_FOLLOWING))};
  });
  assert.equal(example.source,fixture);assert(example.codeBeforeImage);
