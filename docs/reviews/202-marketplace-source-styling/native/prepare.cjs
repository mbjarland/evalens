...
for (const dir of ['workspace', 'bridge', 'native-user-data/User']) {
  fs.mkdirSync(path.join(scratch, dir), { recursive: true });
}
fs.copyFileSync(
  path.join(repo, 'docs/reviews/196-marketplace-page-refresh/fixtures/nested-loops.py'),
...
fs.writeFileSync(path.join(scratch, 'bridge/package.json'), JSON.stringify({
  name: 'evalens-source-review-bridge', publisher: 'local', version: '0.0.1',
